      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 w-full h-[60px] sm:h-[64px] flex items-center justify-between relative">
        ...
        <div className="hidden md:flex items-center space-x-1 lg:space-x-3 xl:space-x-4 ml-auto">
          ...
          >
            ...
            {activeCategory === 'residential' && (
              <div 
                className="absolute top-[100%] right-0 lg:right-4 xl:right-8 w-[950px] max-w-[96vw] h-[435px] max-h-[calc(100vh-90px)] bg-white rounded-2xl shadow-2xl border border-gold/30 z-50 flex overflow-hidden animate-fadeIn origin-top"
                onMouseEnter={() => handleCategoryMouseEnter('residential')}
                onMouseLeave={handleCategoryMouseLeave}
              >
                {/* LEFT SIDE: Single Clean Column of All Locations / Corridors - ALL 16 VISIBLE AT A GLANCE (NO SCROLL, NO CUTOFF) */}
                      ...
                      {RESIDENTIAL_CITIES.map((city) => {
                        ...
                        return (
                          <button
                            ...
                            className={`w-full flex items-center justify-between px-2.5 py-[2px] rounded-md text-xs font-semibold transition-colors duration-100 text-left cursor-pointer ${
                              isHovered
