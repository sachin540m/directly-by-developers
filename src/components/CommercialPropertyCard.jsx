...
const CommercialPropertyCard = ({ property, onEnquire }) => {
  const { 
    id, 
    name, 
    developer, 
  ...
  const cardAnchorId = `commercial-${id}`;
    ...
    >
      ...
      <div className="px-6 py-4 flex-grow flex flex-col">
        ...
        <ul className="space-y-2.5 text-sm text-slate-muted mb-6 flex-grow">
          ...
          <li className="flex items-start gap-2">
            <span className="text-gold mt-0.5 select-none">►</span>
            <span className="font-semibold text-slate-dark">Carpet Area: {carpetArea || bhk || 'Flexible Layouts'}</span>
          </li>
          <li className="flex items-start gap-2">
            <span className="text-gold mt-0.5 select-none">►</span>
        ...
        {/* Action Button: Exact matching Residential Enquire Now */}
        <div className="mt-auto pt-2 w-full">
          <button
            type="button"
            onClick={() => onEnquire && onEnquire(name, city || location)}
            className="w-full bg-gold hover:bg-gold-dark text-white font-bold py-3 px-3 rounded-lg shadow-md hover:shadow-lg transition-all duration-300 uppercase tracking-wider text-[10px] text-center shimmer-sweep cursor-pointer"
          >
            Enquire Now
          </button>
        </div>
